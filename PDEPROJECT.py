...
def growth_rate(u, skip=20000):
    ...
    A = np.sqrt(np.mean(u[skip:]**2, axis=1))   # vector of Amplitudes for each time step
    
    # avoid zeros
    A = np.maximum(A, 1e-20) # avodiing log(0)
    
    t = np.arange(len(A))
    
    coeff = np.polyfit(t, np.log(A), 1) # log(A) = lambda * t + C
...
ax.view_init(elev=0, azim=-0) # angle of the graph
